...
应用启动时由 Flyway 按顺序执行 `backend/platform-system/src/main/resources/db/migration` 中的版本脚本：V1 创建 IAM 表，V2 初始化全局菜单与 19 个固定权限码，V3 创建多实例首租户初始化锁，V4 增加用户认证安全版本及角色反向查询索引，V5 增加菜单内置保护及 7 个菜单管理权限码，V6 增加平台管理员标记与平台/租户权限范围，V7 将平台管理员标记转换为内置 `PLATFORM_ADMIN` 角色关联并移除标记列，V8 清除遗留 `TENANT_ADMIN` 内置标记并使旧会话失效。

新库的 `PLATFORM_ADMIN` 仅授予 `BOOTSTRAP_ADMIN_USERNAME` 初始化的用户；不再预建 `TENANT_ADMIN`，其他角色后续按业务需要在角色管理中创建。V8 保留已有角色及菜单授权，只将遗留 `TENANT_ADMIN` 转为普通角色，不会重新创建已删除角色。普通角色创建和用户角色分配接口禁止创建或授予 `PLATFORM_ADMIN`。用户列表不再展示身份列，接口中的 `platformAdmin` 仅为角色推导的账号保护提示，不是用户持久化字段。V7 升级保留既有平台管理员账号的资格，不会按当前环境变量重新选择账号；修改初始化用户名配置不会转移权限。升级前请备份数据库；V7 会移除旧标记列并使旧会话失效。

仅当数据库中不存在任何租户时，应用才使用 `BOOTSTRAP_*` 创建首租户、平台管理员、唯一的内置 `PLATFORM_ADMIN` 角色及用户角色关联；已有租户时不会再次创建。V6 升级已有数据库时仅将最早租户的首个有效内置租户管理员提升为平台管理员，并清理租户角色中遗留的平台权限关联。迁移脚本是表结构和权限目录的唯一演进入口，请勿修改已发布版本，应新增更高版本脚本。
...
全局菜单管理属于平台级操作，必须同时具备 `system:platform:admin` 身份和对应的 `system:menu:*` 权限；租户管理员可读取租户可分配菜单树，但不能查看或修改全局菜单管理资源。

访问认证接口时使用 HTTP 请求头 `Authorization: Bearer <access-token>`；示例中的 `<access-token>` 是占位符。刷新令牌按会话在 Redis 中保存摘要并轮换，支持同一用户多设备登录。用户状态、密码或权限变化会递增数据库认证版本；即使 Redis 清理暂时失败，旧会话也会在请求和刷新时被拒绝。权限码采用 `领域:资源:动作` 格式，例如 `system:user:list`；`TENANT_ADMIN` 与其他普通角色一样按实际菜单授权计算权限，不再自动获得全权限，也不再受最后一个租户管理员保护；内置 `PLATFORM_ADMIN` 角色自动获得全部已启用权限及 `system:platform:admin` 身份，其他角色只能通过菜单关联获得租户级权限。
