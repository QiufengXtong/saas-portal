...
`sys_user` 保存 `tenant_id`、用户名、密码摘要、显示名称、联系方式、状态、密码修改时间、最后登录时间和认证安全版本。`tenant_id + username` 唯一；用户名更新时必须复用统一身份规范化规则，并递增认证安全版本。登录时仅在密码校验正确后返回用户禁用状态；用户不存在或密码错误仍使用统一凭据错误，避免直接暴露账号是否存在。
